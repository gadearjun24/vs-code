...
def linear_to_db(linear: float, floor_db: float = -80.0) -> float:
    """Inverse of db_to_linear. Silence (linear<=0) returns `floor_db` rather than -inf."""
    if linear <= 1e-8:
        return floor_db
    return float(20.0 * np.log10(linear))


def rms_dbfs(audio: np.ndarray, floor_db: float = -80.0) -> float:
    """
    RMS loudness of `audio` in dBFS (0 dBFS = a full-scale sine wave).
    Used to measure how loud/quiet a speaker's original delivery was
    (shouting vs. whispering) so it can be preserved in the generated dub
    - see `original_volume_dbfs` in step12 and the volume-aware
    normalization in step14.
    """
    if audio is None or audio.size == 0:
        return floor_db
    rms = float(np.sqrt(np.mean(np.square(audio, dtype=np.float64))))
    return linear_to_db(rms, floor_db=floor_db)


def resample_audio(audio: np.ndarray, orig_sr: int, target_sr: int) -> np.ndarray:
    """Resample a mono float32 array. No-op (fast path) if the rates already match."""
    if orig_sr == target_sr or audio.size == 0:
        return audio.astype(np.float32, copy=False)
    from scipy.signal import resample
    target_len = max(1, int(round(len(audio) * target_sr / float(orig_sr))))
