"""
Step 16 - Audio assembly.

Two things happen here:

1. Every generated segment (step14) is placed at its original timestamp.
   As of the current step14, each segment's own .wav is *already*
   ...
   the cases step14's per-segment sync doesn't cover: each line starts at
   ...
   clamped by step14's safety range - a line that still runs long simply
   ...
   default since step14 is now where sync is meant to happen.
...
def place_segments(records: list, mix_sr: int, cfg: PipelineConfig, total_samples: int):
    ...
    step14 (`cfg.sync_segment_duration`), so this is mostly just placing
    already-correct clips at their timestamps; the drift-forward logic
    below only kicks in for whatever step14 didn't fully resolve (sync
    disabled, missing timing data, or a stretch that hit step14's safety
...
def run(cfg: PipelineConfig) -> None:
    ...
    LOG.info(
        ...
        f"(step14 pre-synced most segments to their original duration already"
    ...
    if stats["drifted_lines"]:
        LOG.warning(
            ...
            f"(up to {stats['max_drift_seconds']}s) - likely a segment step14 couldn't fully sync "
